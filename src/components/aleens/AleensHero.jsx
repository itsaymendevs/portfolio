...
export default function AleensHero() {
      ...
      <div className="absolute bottom-0 left-0 right-0 flex flex-col gap-6 px-8 pb-8 max-h-[700px]:gap-4 max-h-[700px]:pb-6 sm:px-10 sm:pb-10 lg:flex-row lg:items-end lg:justify-between lg:px-14 xl:px-16">
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="relative overflow-hidden rounded-full bg-[#f7941d] px-6 py-3 text-sm font-medium text-white shadow-lg max-h-[700px]:px-4 max-h-[700px]:py-2 max-h-[700px]:text-xs [@media((max-width:1024px)_and_(max-height:880px))]:px-4 [@media((max-width:1024px)_and_(max-height:880px))]:py-2 [@media((max-width:1024px)_and_(max-height:880px))]:text-xs"
            style={{
              background:
                "linear-gradient(135deg, #d96c00 0%, #f7941d 45%, #ffb347 80%)",
        ...
        <div className="flex items-end gap-4 self-start lg:self-auto">
          ...
          >
            <div className="relative h-[210px] w-[180px] overflow-hidden rounded-[14px] bg-black lg:h-[340px] lg:w-[300px]">
              {isVideoLoading && (
              ...
              <video
                ref={videoRef}
                src="/aleens/videos/hero-pip.mp4"
                autoPlay
                muted
                loop
                playsInline
