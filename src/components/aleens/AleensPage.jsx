...
import AleensHero from "./AleensHero";
...
import AleensFooter from "./AleensFooter";
import AleensPreloader from "./AleensPreloader";
import FloatingActions from "./FloatingActions";

export default function AleensPage() {
  ...
  useEffect(() => {
    ...
    document.title =
      "Aleens — Fresh Meal Plans & Daily Delivery Across UAE | Dubai, Abu Dhabi, Sharjah";
    ...
    const mDesc = upsert(
      "name",
      "description",
      "Aleens crafts personalized meal plans — balanced, high-protein, plant-based — with fresh daily delivery across Dubai, Abu Dhabi & Sharjah. Chef-crafted, dietitian-approved, flexible subscriptions.",
    );
    const mOgTitle = upsert(
      "property",
      "og:title",
      "Aleens — Personalized Meal Plans in UAE",
    );
    ...
    const mOgImg = upsert("property", "og:image", "/aleens/images/hero.jpg");
    ...
    if (fav) fav.href = "/aleens/images/logo.png";
  ...
  useEffect(() => {
    if (!ready) return;
    document.documentElement.classList.add("aleens-html");
    document.body.classList.add("aleens");
    ...
    return () => {
      ...
      lenis.destroy();
      document.documentElement.classList.remove("aleens-html");
      document.body.classList.remove("aleens");
    };
  }, [ready]);
  ...
  return (
    <div className="aleens min-h-screen bg-white">
      {preloading && <AleensPreloader onDone={() => setPreloading(false)} />}
      {ready && (
        <>
          <Navbar />
          <AleensHero />
