...
const DESC_MOBILE_HOW = "— Simple steps to get started, choose your plan, customize your meals, and let us handle the rest.";

const STEPS = [
  {
    number: "01",
    img: "/behealthy/images/how-it-work-1.png",
    lines: ["Pick your perfect plan —", "balanced, high-protein, or", "plant-based for you."],
  },
  {
    number: "02",
    img: "/behealthy/images/how-it-work-2.png",
    lines: ["Select favorite dishes,", "set excludes and", "preferences with ease."],
  },
  {
    number: "03",
    img: "/behealthy/images/how-it-work-3.png",
    lines: ["Fresh meals delivered,", "track progress, pause or", "extend anytime."],
  },
];
    ...
    const obs = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting) {
          ...
          STEPS.forEach((_, i) =>
            setTimeout(() => setCardsVisible((v) => v + 1), afterDesc + i * 180),
          );
          obs.disconnect();
        }
      },
    ...
    >
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-12 lg:flex-row lg:items-start lg:justify-between lg:gap-10">
        ...
        <div className="relative flex w-full justify-center lg:w-[68%] lg:justify-end">
          ...
          >
            <svg viewBox="0 0 720 132" fill="none" preserveAspectRatio="none" className="h-full w-full overflow-visible">
          ...
          <motion.div
            ...
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
          >
            <svg viewBox="0 0 300 60" fill="none" preserveAspectRatio="none" className="h-full w-full">
          ...
          <motion.div
            ...
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
          >
            <svg viewBox="0 0 360 620" fill="none" preserveAspectRatio="none" className="h-full w-full overflow-visible">
            ...
            {STEPS.map((step, idx) => (
              <motion.div
                ...
                animate={idx < cardsVisible ? { opacity: 1, y: 0, scale: 1 } : {}}
                transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                className={`relative flex w-[280px] shrink-0 flex-col items-center rounded-[20px] border border-black/[0.06] bg-white/70 p-6 shadow-[0_8px_24px_rgba(0,0,0,0.04)] backdrop-blur-[8px] sm:w-[260px] lg:w-[228px] xl:w-[264px] ${
                  idx === 0 ? "max-sm:self-start" : idx === 1 ? "max-sm:self-end" : "max-sm:self-start"
                }`}
                style={{
              ...
              >
                ...
                <div className="mt-2 flex h-[88px] w-[88px] items-center justify-center">
                  <img src={step.img} alt={`Step ${step.number}`} className="h-[72px] w-[72px] object-contain" />
                </div>
